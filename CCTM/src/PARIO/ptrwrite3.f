...
C Get file description.
     ...
     &           // ' file description'
             CALL M3WARN( 'PTRWRITE3_X', JDATE, JTIME, MSG )
         END IF
...
C Operation valid only for gridded files.
     ...
     &        UPNAM3D .EQ. 'FAKE_GRIDDED' ) THEN
            ...
            IF ( MY_PE .EQ. IO_PE ) THEN
               IF ( .NOT. WRITE3 ( FNAME, VNAME, JDATE, JTIME, REAL ( BUFFER%x , 4 ) ) ) THEN
...
C Loop over file variables.
     ...
     &                  // TRIM( VNAM16 ) //
     &                  ' to file '// TRIM( FIL16 )
                  CALL PM3WARN( 'PTRWRITE3_X', JDATE, JTIME, MSG )
                  PTRWRITE3_X = .FALSE.; RETURN
               END IF
      ...
      ELSE     ! This is not a gridded file.
         ...
         MSG = 'PTRWRITE3_X can not handle this type of data.'
         CALL PM3WARN( 'PTRWRITE3', JDATE, JTIME, MSG )
         PTRWRITE3_X = .FALSE.; RETURN

      END IF   ! gridded file

      CALL MPI_BCAST ( PTRWRITE3_X, 1, MPI_LOGICAL, 0, MPI_COMM_WORLD, ERROR )

      IF ( .NOT. PTRWRITE3_X ) THEN            !  failure
         IF ( MY_PE .EQ. IO_PE ) THEN
...
C Get file description.
         IF ( .NOT. DESC3( FNAME ) ) THEN
             MSG = 'Could not get '// TRIM( FNAME )
     &           // ' file description'
             CALL M3WARN( 'PTRWRITE3_DX1', JDATE, JTIME, MSG )
         END IF
...
C Operation valid only for gridded files.
     ...
     &        UPNAM3D .EQ. 'FAKE_GRIDDED' ) THEN
            ...
            IF ( MY_PE .EQ. IO_PE ) THEN
               IF ( .NOT. WRITE3 ( FNAME, VNAME, JDATE, JTIME, REAL ( BUFFER%dx1 , 4 ) ) ) THEN
                  MSG = 'WRITE3 failed writing variable '
     &                // TRIM( VNAME )
     &                // ' to file '// TRIM( FNAME )
...
C Loop over file variables.
            DO IV = 1, NVARS
               ...
               IF ( .NOT. PWRGRDD( FIL16, VNAM16, JDATE, JTIME,
     &                             REAL ( BUFFER( :,:,:,IV )%dx1 , 4 ),
     &                             NCOLS3D, NROWS3D, NLAYS3D,
     &                             NUMCOLS, NUMROWS, NUMPROCS ) ) THEN
                  MSG = 'PWRGRDD failed writing variable '
     &                  // TRIM( VNAM16 ) //
     &                  ' to file '// TRIM( FIL16 )
                  CALL PM3WARN( 'PTRWRITE3_DX1', JDATE, JTIME, MSG )
                  PTRWRITE3_DX1 = .FALSE.; RETURN
               END IF
      ...
      ELSE     ! This is not a gridded file.
         ...
         MSG = 'PTRWRITE3_DX1 can not handle this type of data.'
         CALL PM3WARN( 'PTRWRITE3_DX1', JDATE, JTIME, MSG )
         PTRWRITE3_DX1 = .FALSE.; RETURN

      END IF   ! gridded file

      CALL MPI_BCAST ( PTRWRITE3_DX1, 1, MPI_LOGICAL, 0, MPI_COMM_WORLD, ERROR )

      IF ( .NOT. PTRWRITE3_DX1 ) THEN            !  failure
         IF ( MY_PE .EQ. IO_PE ) THEN
            IF ( TSTEP3( FID ) .EQ. 0 ) THEN
               WRITE( LOGDEV, 92030 ) 'Error writing', VNAME, 'to', FNAME
            ELSE
...
C Get file description.
         IF ( .NOT. DESC3( FNAME ) ) THEN
             MSG = 'Could not get '// TRIM( FNAME )
     &           // ' file description'
             CALL M3WARN( 'PTRWRITE3_DX2', JDATE, JTIME, MSG )
         END IF
...
C Operation valid only for gridded files.
     ...
     &        UPNAM3D .EQ. 'FAKE_GRIDDED' ) THEN
            ...
            IF ( MY_PE .EQ. IO_PE ) THEN
               IF ( .NOT. WRITE3 ( FNAME, VNAME, JDATE, JTIME, REAL ( BUFFER%dx2 , 4 ) ) ) THEN
                  MSG = 'WRITE3 failed writing variable '
     &                // TRIM( VNAME )
     &                // ' to file '// TRIM( FNAME )
                  CALL M3WARN( 'PTRWRITE3_DX2', JDATE, JTIME, MSG )
...
C Loop over file variables.
            DO IV = 1, NVARS
               ...
               IF ( .NOT. PWRGRDD( FIL16, VNAM16, JDATE, JTIME,
     &                             REAL ( BUFFER( :,:,:,IV )%dx2 , 4 ),
     &                             NCOLS3D, NROWS3D, NLAYS3D,
     &                             NUMCOLS, NUMROWS, NUMPROCS ) ) THEN
                  MSG = 'PWRGRDD failed writing variable '
     &                  // TRIM( VNAM16 ) //
     &                  ' to file '// TRIM( FIL16 )
                  CALL PM3WARN( 'PTRWRITE3_DX2', JDATE, JTIME, MSG )
                  PTRWRITE3_DX2 = .FALSE.; RETURN
               END IF
      ...
      ELSE     ! This is not a gridded file.
         ...
         MSG = 'PTRWRITE3_DX2 can not handle this type of data.'
         CALL PM3WARN( 'PTRWRITE3_DX2', JDATE, JTIME, MSG )
         PTRWRITE3_DX2 = .FALSE.; RETURN

      END IF   ! gridded file

      CALL MPI_BCAST ( PTRWRITE3_DX2, 1, MPI_LOGICAL, 0, MPI_COMM_WORLD, ERROR )

      IF ( .NOT. PTRWRITE3_DX2 ) THEN            !  failure
         IF ( MY_PE .EQ. IO_PE ) THEN
            IF ( TSTEP3( FID ) .EQ. 0 ) THEN
               WRITE( LOGDEV, 92030 ) 'Error writing', VNAME, 'to', FNAME
            ELSE
...
C Check that Models-3 I/O has been initialized:
      ...
      PTRWRITE3_DX1X2 = .TRUE.

      IF ( .NOT. FINIT3 ) THEN
         IDUM = INIT3()
         WRITE( LOGDEV,91010 ) 'WRITE3():  I/O API not yet initialized.'
         PTRWRITE3_DX1X2 = .FALSE.; RETURN
      END IF
...
C Get file description.
         IF ( .NOT. DESC3( FNAME ) ) THEN
             MSG = 'Could not get '// TRIM( FNAME )
     &           // ' file description'
             CALL M3WARN( 'PTRWRITE3_DX1X2', JDATE, JTIME, MSG )
         END IF
...
C Operation valid only for gridded files.
     ...
     &        UPNAM3D .EQ. 'FAKE_GRIDDED' ) THEN
            ...
            IF ( MY_PE .EQ. IO_PE ) THEN
               IF ( .NOT. WRITE3 ( FNAME, VNAME, JDATE, JTIME, REAL ( BUFFER%dx1x2 , 4 ) ) ) THEN
                  MSG = 'WRITE3 failed writing variable '
     &                // TRIM( VNAME )
     &                // ' to file '// TRIM( FNAME )
                  CALL M3WARN( 'PTRWRITE3_DX1X2', JDATE, JTIME, MSG )
                  PTRWRITE3_DX1X2 = .FALSE.
               END IF
            END IF
...
C Loop over file variables.
            DO IV = 1, NVARS
               ...
               IF ( .NOT. PWRGRDD( FIL16, VNAM16, JDATE, JTIME,
     &                             REAL ( BUFFER( :,:,:,IV )%dx1x2 , 4 ),
     &                             NCOLS3D, NROWS3D, NLAYS3D,
     &                             NUMCOLS, NUMROWS, NUMPROCS ) ) THEN
                  MSG = 'PWRGRDD failed writing variable '
     &                  // TRIM( VNAM16 ) //
     &                  ' to file '// TRIM( FIL16 )
                  CALL PM3WARN( 'PTRWRITE3_DX1X2', JDATE, JTIME, MSG )
                  PTRWRITE3_DX1X2 = .FALSE.; RETURN
               END IF
      ...
      ELSE     ! This is not a gridded file.
         ...
         MSG = 'PTRWRITE3_DX1X2 can not handle this type of data.'
         CALL PM3WARN( 'PTRWRITE3_DX1X2', JDATE, JTIME, MSG )
         PTRWRITE3_DX1X2 = .FALSE.; RETURN

      END IF   ! gridded file

      CALL MPI_BCAST ( PTRWRITE3_DX1X2, 1, MPI_LOGICAL, 0, MPI_COMM_WORLD, ERROR )

      IF ( .NOT. PTRWRITE3_DX1X2 ) THEN            !  failure
         IF ( MY_PE .EQ. IO_PE ) THEN
            IF ( TSTEP3( FID ) .EQ. 0 ) THEN
               WRITE( LOGDEV, 92030 ) 'Error writing', VNAME, 'to', FNAME
            ELSE
